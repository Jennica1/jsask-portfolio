...
export default function FloatingShapes() {
  ...
  useEffect(() => {
    ...
    // Prevent right-click from dragging objects
    sceneRef.current.addEventListener("contextmenu", (e) => e.preventDefault());
  
    // Listen for mouse events
    document.addEventListener("mousedown", (e) => {
      if (e.button !== 0) return; // Only allow left-click to interact
      Matter.Body.setVelocity(floatingTriangleRef.current, { x: 2, y: -2 });
    });
  
    document.addEventListener("mouseup", (e) => {
      if (e.button === 2) {
        mouseConstraint.body = null; // Release any held body when right-click is released
      }
    });
