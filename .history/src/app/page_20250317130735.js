...
export default function FloatingShapes() {
  ...
  useEffect(() => {
    ...
    const floatingCircle = Matter.Bodies.circle(startX + 100, startY + 100, 50, {
      restitution: 1,
      frictionAir: 0,
      density: 0.002,
    });
    ...
    Matter.Body.setVelocity(floatingS, { x: (Math.random() - 0.5) * 2, y: (Math.random() - 0.5) * 2 });
    ...
    const updateImagePositions = () => {
      ...
      if (starRef.current && floatingCircleRef.current) {
        const { x, y } = floatingCircleRef.current.position;
        circleRef.current.style.transform = `translate(${x - 50}px, ${y - 50}px)`;
      }
