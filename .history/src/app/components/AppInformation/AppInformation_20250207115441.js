...
export default function AppInformation({ appName, appLink, role, skills, description, status }) {
  return (
    <section>
      <div className="flex flex-row gap-8"> {/* Added flex and gap */}
        {/* Left Section */}
        <div className="text-sm  sm:w-2/3 md:w-3/4 md:text-xs">
          <span className="inline-flex items-baseline pb-1">
            <h2 className="text-3xl font-bold md:text-lg mr-2">{appName}</h2>
            <Link
              href={appLink}
              target="_blank"
