...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const floatingImage = Matter.Bodies.circle(startX, startY, 60, {
      restitution: 0.8, // Bouncy effect
      frictionAir: 0.05, // Makes it float
    ...
    Matter.Body.applyForce(floatingImage, floatingImage.position, {
      x: (Math.random() - 0.5) * 0.02, // Random nudge
      y: (Math.random() - 0.5) * 0.02,
    });

    // Keep applying random forces to make it float
    ...
    const interval = setInterval(applyRandomForce, 2000);
